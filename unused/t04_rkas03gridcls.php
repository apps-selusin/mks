...
$t04_rkas03_grid = NULL; // Initialize page object first

class ct04_rkas03_grid extends ct04_rkas03 {
	...
	// Page object name
	var $PageObjName = 't04_rkas03_grid';

	// Grid form hidden field names
	var $FormName = 'ft04_rkas03grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t04_rkas03"];
	...
	function Page_Init() {
		...
		if (@$_POST["ajax"] == "autofill") {
			...
			if ($sDetailTblVar <> "") {
				...
				if (in_array("t05_rkas04", $DetailTblVar)) {

					// Process auto fill for detail table 't05_rkas04'
					if (preg_match('/^ft05_rkas04(grid|add|addopt|edit|update|search)$/', @$_POST["form"])) {
						if (!isset($GLOBALS["t05_rkas04_grid"])) $GLOBALS["t05_rkas04_grid"] = new ct05_rkas04_grid;
						$GLOBALS["t05_rkas04_grid"]->Page_Init();
						$this->Page_Terminate();
						exit();
					}
				}
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t03_rkas02") {
			global $t03_rkas02;
			$rsmaster = $t03_rkas02->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("t03_rkas02list.php"); // Return to master page
			} else {
				$t03_rkas02->LoadListRowValues($rsmaster);
				$t03_rkas02->RowType = EW_ROWTYPE_MASTER; // Master row
				$t03_rkas02->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function RenderRow() {
		...
		$this->jumlah->ViewCustomAttributes = "";
			...
			if ($this->lv2_id->getSessionValue() <> "") {
				$this->lv2_id->CurrentValue = $this->lv2_id->getSessionValue();
				$this->lv2_id->OldValue = $this->lv2_id->CurrentValue;
			if ($this->lv2_id->VirtualValue <> "") {
				$this->lv2_id->ViewValue = $this->lv2_id->VirtualValue;
			} else {
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "t03_rkas02") {
				$this->lv2_id->CurrentValue = $this->lv2_id->getSessionValue();
			}
		$conn = &$this->Connection();
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "t03_rkas02") {
			$this->lv2_id->Visible = FALSE;
			if ($GLOBALS["t03_rkas02"]->EventCancelled) $this->EventCancelled = TRUE;
		}
