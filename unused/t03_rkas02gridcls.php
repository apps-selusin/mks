...
$t03_rkas02_grid = NULL; // Initialize page object first

class ct03_rkas02_grid extends ct03_rkas02 {
	...
	// Page object name
	var $PageObjName = 't03_rkas02_grid';

	// Grid form hidden field names
	var $FormName = 'ft03_rkas02grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t03_rkas02"];
	...
	function Page_Init() {
		...
		if (@$_POST["ajax"] == "autofill") {
			...
			if ($sDetailTblVar <> "") {
				...
				if (in_array("t04_rkas03", $DetailTblVar)) {

					// Process auto fill for detail table 't04_rkas03'
					if (preg_match('/^ft04_rkas03(grid|add|addopt|edit|update|search)$/', @$_POST["form"])) {
						if (!isset($GLOBALS["t04_rkas03_grid"])) $GLOBALS["t04_rkas03_grid"] = new ct04_rkas03_grid;
						$GLOBALS["t04_rkas03_grid"]->Page_Init();
						$this->Page_Terminate();
						exit();
					}
				}
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t02_rkas01") {
			global $t02_rkas01;
			$rsmaster = $t02_rkas01->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("t02_rkas01list.php"); // Return to master page
			} else {
				$t02_rkas01->LoadListRowValues($rsmaster);
				$t02_rkas01->RowType = EW_ROWTYPE_MASTER; // Master row
				$t02_rkas01->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function RenderRow() {
		...
		$this->jumlah->ViewCustomAttributes = "";
			...
			if ($this->lv1_id->getSessionValue() <> "") {
				$this->lv1_id->CurrentValue = $this->lv1_id->getSessionValue();
				$this->lv1_id->OldValue = $this->lv1_id->CurrentValue;
			if ($this->lv1_id->VirtualValue <> "") {
				$this->lv1_id->ViewValue = $this->lv1_id->VirtualValue;
			} else {
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "t02_rkas01") {
				$this->lv1_id->CurrentValue = $this->lv1_id->getSessionValue();
			}
		$conn = &$this->Connection();
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "t02_rkas01") {
			$this->lv1_id->Visible = FALSE;
			if ($GLOBALS["t02_rkas01"]->EventCancelled) $this->EventCancelled = TRUE;
		}
