...
$t05_rkas04_grid = NULL; // Initialize page object first

class ct05_rkas04_grid extends ct05_rkas04 {
	...
	// Page object name
	var $PageObjName = 't05_rkas04_grid';

	// Grid form hidden field names
	var $FormName = 'ft05_rkas04grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t05_rkas04"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t04_rkas03") {
			global $t04_rkas03;
			$rsmaster = $t04_rkas03->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("t04_rkas03list.php"); // Return to master page
			} else {
				$t04_rkas03->LoadListRowValues($rsmaster);
				$t04_rkas03->RowType = EW_ROWTYPE_MASTER; // Master row
				$t04_rkas03->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function RenderRow() {
		...
		$this->jumlah->ViewCustomAttributes = "";
			...
			if ($this->lv3_id->getSessionValue() <> "") {
				$this->lv3_id->CurrentValue = $this->lv3_id->getSessionValue();
				$this->lv3_id->OldValue = $this->lv3_id->CurrentValue;
			if ($this->lv3_id->VirtualValue <> "") {
				$this->lv3_id->ViewValue = $this->lv3_id->VirtualValue;
			} else {
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "t04_rkas03") {
				$this->lv3_id->CurrentValue = $this->lv3_id->getSessionValue();
			}
		$conn = &$this->Connection();
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "t04_rkas03") {
			$this->lv3_id->Visible = FALSE;
			if ($GLOBALS["t04_rkas03"]->EventCancelled) $this->EventCancelled = TRUE;
		}
